    ...
    <div v-else-if="tab === 'edit'" class="space-y-4">
      ...
      <div class="grid sm:grid-cols-2 gap-3">
        ...
        <label class="block">
          ...
          <input v-model="form.releaseDate" type="date" class="field" :min="minDate" :disabled="dateLocked" :class="{ 'opacity-50 cursor-not-allowed': dateLocked }" @change="clampReleaseDate" />
      ...
      <div v-for="(tr, idx) in form.tracks" :key="tr.localId" class="border border-[#333] p-4 space-y-3">
        ...
        <div>
          <span class="lbl">Жанры трека</span>
...
const genreDisplay = computed(() => {
  const tr = track.value as any
  if (!tr) return '—'
  const fromTracks = (tr.tracksDetail || []).flatMap((x: any) => x.genres || [])
  const uniq = [...new Set(fromTracks)]
  if (uniq.length) return uniq.join(', ')
  if (tr.genres?.length) return tr.genres.join(', ')
  return tr.genre || '—'
})
...
function syncForm() {
  ...
  for (const tr of form.tracks) {
    if (!tr.genres) (tr as any).genres = []
  }
  if (!listenId.value && form.tracks[0]) listenId.value = form.tracks[0].localId
}
watch(track, syncForm, { immediate: true })

function clampReleaseDate() {
  if (form.releaseDate && form.releaseDate < minDate) form.releaseDate = minDate
}
...
function save() {
  ...
  const allGenres = [...new Set(form.tracks.flatMap((tr) => tr.genres || []))]
  const patch: any = {
    title: form.title,
    type: form.type,
