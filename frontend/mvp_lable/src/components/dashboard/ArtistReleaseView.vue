  ...
  <section v-if="track" class="space-y-4 border-2 border-[#39FF14] bg-[#050505] p-4 sm:p-6">
    ...
    <div v-else-if="tab === 'listen'" class="space-y-4">
      ...
      <template v-if="listenTrack">
        ...
        <pre class="whitespace-pre-wrap font-mono text-sm text-gray-200 border border-[#333] p-4 bg-black">{{ listenTrack.lyrics || '' }}</pre>
      </template>
    </div>

    <div v-else-if="tab === 'stats'" class="space-y-3">
    ...
    <div v-else-if="tab === 'edit'" class="space-y-4">
      ...
      <div>
        ...
        <div class="flex flex-wrap items-center gap-3 mt-1">
          <img v-if="form.coverUrl" :src="form.coverUrl" alt="cover" class="w-24 h-24 object-cover border-2 border-[#333]" />
          <label class="btn-muted cursor-pointer inline-flex items-center">
            Загрузить обложку
      ...
      <div class="grid sm:grid-cols-2 gap-3">
        ...
        <label class="block">
          ...
          <input v-model="form.releaseDate" type="date" class="field" :disabled="dateLocked" :class="{ 'opacity-50 cursor-not-allowed': dateLocked }" />
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2 mt-2">
          <label v-for="g in GENRES" :key="g" class="flex items-center gap-2 border border-[#333] px-2 py-1.5 cursor-pointer font-mono text-[10px] uppercase" :class="{ 'border-[#39FF14] text-[#39FF14]': form.genres.includes(g) }">
            <input type="checkbox" :value="g" v-model="form.genres" class="accent-[#39FF14]" />{{ g }}
          </label>
        </div>
      </div>
        ...
        <div>
          ...
          <div class="mt-1 space-y-2">
            ...
            <label class="btn-muted cursor-pointer inline-flex items-center">
              ...
              <input type="file" accept="audio/*" class="hidden" @change="(e) => onAudio(e, idx)" />
            </label>
          </div>
        </div>
        <div>
          ...
          <textarea v-model="tr.lyrics" rows="10" class="field font-mono text-sm" />
        </div>
      </div>
...
const GENRES = ['electronic','synthwave','hip-hop','r&b','pop','rock','bass','experimental','ambient','drill','phonk','metal','indie','trap']
...
function onCover(e: Event) {
  ...
  if (f) form.coverUrl = URL.createObjectURL(f)
}
function onAudio(e: Event, idx: number) {
  const f = (e.target as HTMLInputElement).files?.[0]
  if (!f || !form.tracks[idx]) return
...
function save() {
  ...
  if (!dateLocked.value) patch.releaseDate = form.releaseDate
