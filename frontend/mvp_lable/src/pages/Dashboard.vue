    ...
    <div class="max-w-7xl mx-auto flex flex-col gap-12 relative z-10">
      ...
      <section class="data-panel-container">
        ...
        <div class="data-panel-content">
          ...
          <div class="stat-item"><span class="label">Followers</span><span class="value">{{ displayedFollowers.toLocaleString() }}</span></div>
...
.data-panel-content { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1px; background: #333; }
...
const platformKey = computed(() => {
  ...
  return map[activePlatform.value] || null
})

const displayedPlays = computed(() => {
  if (!platformKey.value) return tracksStore.totalPlays
  return tracksStore.tracks.reduce((s, tr) => s + (tr.platforms?.[platformKey.value!] ?? 0), 0)
})

const displayedFollowers = computed(() => {
  if (!platformKey.value) {
    return tracksStore.tracks.reduce((s, tr) => {
      const f = tr.followers
      if (!f) return s
      return s + (f.spotify || 0) + (f.apple || 0) + (f.yandex || 0) + (f.vk || 0)
    }, 0)
  }
  return tracksStore.tracks.reduce((s, tr) => s + (tr.followers?.[platformKey.value!] ?? 0), 0)
})
